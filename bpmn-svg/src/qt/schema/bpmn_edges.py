...
class BpmnEdges(CollapsibleFrame):
    ...
    def on_new_edge(self, index=0):
        # if there are less than two lanes, we can not have bpmn level edges
        if len(self.bpmn_data['lanes']) < 2:
            return
        ...
        self.bpmn_data['edges'].insert(idx, new_edge_object)
    ...
    def on_remove_lane(self, lane_id):
        ...
        self.bpmn_data['edges'] = [x for x in self.edges if not associated_with_lane(x, lane_id)]
        self.edges = self.bpmn_data['edges']
        self.populate()

    def on_remove_pool(self, pool_id):
        print(type(self).__name__, 'remove_pool', pool_id)
        self.bpmn_data['edges'] = [x for x in self.edges if not associated_with_pool(x, pool_id)]
        self.edges = self.bpmn_data['edges']
        self.populate()

    def on_remove_node(self, node_id):
        print(type(self).__name__, 'remove_node', node_id)
        self.bpmn_data['edges'] = [x for x in self.edges if not associated_with_node(x, node_id)]
        self.edges = self.bpmn_data['edges']
        self.populate()
