...
class LanePools(CollapsibleFrame):
    def __init__(self, bpmn_data, bpmn_id, lane_id, lane_pools, parent=None):
        super().__init__(icon='pools', text='Lane Pools', parent=parent)
        self.bpmn_data, self.bpmn_id, self.lane_id, self.lane_pools = bpmn_data, bpmn_id, lane_id, lane_pools
    ...
    def populate(self):
        ...
        for pool_id, pool_data in self.lane_pools.items():
            pool_widget = PoolEditor(self.bpmn_data, self.bpmn_id, self.lane_id, pool_id, pool_data)
