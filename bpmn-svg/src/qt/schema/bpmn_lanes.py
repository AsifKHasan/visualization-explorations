...
class BpmnLanes(CollapsibleFrame):
    def __init__(self, bpmn_data, bpmn_id, lanes, parent=None):
        super().__init__(icon='lanes', text='BPMN Lanes', parent=parent)
        self.bpmn_data, self.bpmn_id, self.lanes = bpmn_data, bpmn_id, lanes
    ...
    def populate(self):
        ...
        for lane_id, lane_data in self.lanes.items():
            lane_widget = LaneEditor(self.bpmn_data, self.bpmn_id, lane_id, lane_data)
