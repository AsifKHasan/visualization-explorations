...
class EventIntermediateCatchCompensation(EventIntermediate):
    ...
    def __init__(self, current_theme, bpmn_id, lane_id, pool_id, node_id, node_data):
        ...
        self.theme.update(self.current_theme['events']['intermediates']['EventIntermediateCatchCompensation'])

    def get_inside_element(self):
        svg_group, group_width, group_height = a_triangular_rewind_inside_a_circular_shape(radius=self.theme['inner-circle']['radius'], inner_shape_spec=self.theme['inner-shape'])
