...
class ActivityEventEscalationNon(ActivityEventSubprocess):
    ...
    def __init__(self, current_theme, bpmn_id, lane_id, pool_id, node_id, node_data):
        ...
        self.theme = {**self.theme, **self.current_theme['activities']['subprocesses']['event-subprocesses']['ActivityEventEscalationNon']}

    def get_top_left_element(self):
        inner_svg, inner_width, inner_height = an_upword_arrowhead_inside_a_circular_shape(radius=self.theme['top-left-circle']['radius'], inner_shape_spec=self.theme['top-left-inner-shape'])
