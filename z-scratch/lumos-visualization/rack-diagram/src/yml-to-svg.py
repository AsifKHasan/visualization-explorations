...
from block.block_helper import BlockHelper
...
class SvgFromYml(object):
	...
	def run(self):
		...
		# dot-helper
		self._CONFIG['files']['output-svg'] = f"{self._CONFIG['dirs']['output-dir']}/{self._yml_name}.svg"
		block_helper = BlockHelper(self._CONFIG)
		block_helper.generate_and_save(self._data)
		self.tear_down()
	...
	def set_up(self):
		...
		self._CONFIG['dirs']['template-dir'] = config_dir / 'templates'
		self._CONFIG['dirs']['svg-template-dir'] = config_dir / 'templates' / 'svg'
